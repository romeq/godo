...
func main() {
	...
	if getArg(1) == "--show-amount" {
		amount, err := database.GetTodosAmount()
		utils.Check(err)
		if getArg(2) == "" {
			fmt.Printf("You have %d To-Dos left \n", amount)
		} else if getArg(2) == "--output-raw" {
			fmt.Println(amount)
		} else {
		...
		fmt.Println("./godo [--show-amount [--output-raw]]")
	}
